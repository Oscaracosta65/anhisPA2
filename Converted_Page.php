...
if (isset($stateName, $gName) && $stateName !== '' && $gName !== '') {
    ...
    $metaDesc = 'Verified draw frequency, recency, and historical analysis for ' . $stateName . ' ' . $gName . '. Review the most active numbers, quiet stretches, and full distribution before SKAI analysis.';
...
function leFetchRecentDraws(\Joomla\Database\DatabaseDriver $db, string $dbCol, string $gameId, int $limit): array
{
    $query = $db->getQuery(true)
        ->select([
            ...
            $db->quoteName('fourth'),
            $db->quoteName('fifth'),
            $db->quoteName('sixth'), // retained for schema compatibility; not used in Pick 5 analysis
        ])
...
[$mainCounts, $mainLastSeenIndex] = leInitRange(1, 43);
 ...
 * --------------------------------------------------------------------------
 */
$mainChartLabels  = leBuildNaturalLabels(1, 43);
$mainChartValues  = [];
$mainRecencyValues = [];
...
[$counts50, ]  = leInitRange(1, 43);
[$counts300, ] = leInitRange(1, 43);
...
/**
 * --------------------------------------------------------------------------
 * Draw history summary for current draw (main balls only)
 * --------------------------------------------------------------------------
 */
$drawHistoryRows = [];
...
/**
 * --------------------------------------------------------------------------
 * Game-specific URL helpers
 * --------------------------------------------------------------------------
 */
$skaiUrlVal = '/picking-winning-numbers/artificial-intelligence/skai-lottery-prediction?gameId=' . rawurlencode($gameId);
$aiUrlVal   = '/picking-winning-numbers/artificial-intelligence/ai-powered-predictions?game_id=' . rawurlencode($gameId);
$skipUrlVal = '/picking-winning-numbers/artificial-intelligence/skip-and-hit-analysis?game_id=' . rawurlencode($gameId);
$mcmcUrlVal = '/picking-winning-numbers/artificial-intelligence/markov-chain-monte-carlo-mcmc-analysis?game_id=' . rawurlencode($gameId);

if ($gameId === 'PA2') {
    $heatmapUrlVal = '/picking-winning-numbers/artificial-intelligence/pennsylvania-cash-5-heatmap-analysis';
} elseif ($gameId === 'NC1') {
    $heatmapUrlVal = '/picking-winning-numbers/artificial-intelligence/north-carolina-cash-5-heatmap-analysis';
} else {
    $heatmapUrlVal = '/all-lottery-heatmaps?gId=' . rawurlencode($gameId)
        . '&stateName=' . rawurlencode((string) ($stateName ?? ''))
        . '&gName='     . rawurlencode((string) ($gName     ?? ''))
        . '&sTn='       . rawurlencode(strtolower((string) ($stateAbrev ?? '')));
}

$archiveUrlVal = '/lottery-archives-pick5?gId=' . rawurlencode($gameId)
    . '&stateName=' . rawurlencode((string) ($stateName ?? ''))
    . '&gName='     . rawurlencode((string) ($gName     ?? ''))
    . '&sTn='       . rawurlencode(strtolower((string) ($stateAbrev ?? '')));

$lowestUrlVal = '/lowest-drawn-number-analysis?gId=' . rawurlencode($gameId)
    . '&stateName=' . rawurlencode((string) ($stateName ?? ''))
    . '&gName='     . rawurlencode((string) ($gName     ?? ''))
    . '&sTn='       . rawurlencode(strtolower((string) ($stateAbrev ?? '')));

$formActionVal = '/all-us-lotteries/results-analysis?st='
    . htmlspecialchars((string) ($stateAbrev ?? ''), ENT_QUOTES, 'UTF-8')
    . '&amp;stn=' . htmlspecialchars((string) ($stateName ?? ''), ENT_QUOTES, 'UTF-8')
    . '&amp;gm='  . htmlspecialchars((string) ($gName     ?? ''), ENT_QUOTES, 'UTF-8');
 ...
 * --------------------------------------------------------------------------
 */
$jsonLdPageTitle = isset($stateName, $gName)
    ? ($stateName . ' ' . $gName . ' Results Intelligence | LottoExpert.net')
    : 'Results Intelligence | LottoExpert.net';

$jsonLdPage = json_encode([
    '@context'    => 'https://schema.org',
    '@type'       => 'WebPage',
    'name'        => $jsonLdPageTitle,
...
$jsonLdDataset = json_encode([
    '@context'    => 'https://schema.org',
    '@type'       => 'Dataset',
    'name'        => (isset($stateName, $gName) ? ($stateName . ' ' . $gName) : 'Lottery') . ' Draw Frequency Analysis',
    'description' => 'Historical draw frequency and recency analysis for '
        . (isset($stateName, $gName) ? ($stateName . ' ' . $gName) : 'this lottery') . '.',
    'url'         => $canonicalNoQuery,
], JSON_UNESCAPED_SLASHES | JSON_UNESCAPED_UNICODE);
?>
<style>
      ...
      <div class="skai-hero-top">
        ...
        <div class="skai-hero-copy">
          ...
          <div class="skai-hero-actions" aria-label="Primary actions">
            <a class="skai-btn skai-btn--primary" href="<?php echo htmlspecialchars($skaiUrlVal, ENT_QUOTES, 'UTF-8'); ?>">
              Open SKAI Analysis
            </a>
            <a class="skai-btn skai-btn--secondary" href="<?php echo htmlspecialchars($aiUrlVal, ENT_QUOTES, 'UTF-8'); ?>">
              AI Predictions
            </a>
          ...
          <div class="skai-advanced-links" aria-label="Advanced tools">
            <a class="skai-mini-link" href="<?php echo htmlspecialchars($skipUrlVal, ENT_QUOTES, 'UTF-8'); ?>">Skip &amp; Hit Analysis</a>
            <a class="skai-mini-link" href="<?php echo htmlspecialchars($mcmcUrlVal, ENT_QUOTES, 'UTF-8'); ?>">MCMC Markov Analysis</a>
            <a class="skai-mini-link" href="<?php echo htmlspecialchars($heatmapUrlVal, ENT_QUOTES, 'UTF-8'); ?>">Heatmap Analysis</a>
          </div>
        </div>
  ...
  <section id="frequency-deep-dive" class="skai-section" aria-labelledby="frequency-title">
    <div class="skai-section-head">
      <div>
        ...
        <p class="skai-section-sub">
          Move from summary to full reference. The left panel shows the complete main-number distribution across all values 01&ndash;43. The right panel shows how recency (distance since last appearance) is distributed across the same number pool.
        </p>
      </div>
    </div>
    ...
    <div class="skai-section-body">
      <div class="skai-two-col">
        <div class="skai-card">
          <div class="skai-card-head skai-card-head--horizon">
            ...
            <span class="skai-card-sub">All values 01&ndash;43 across the last <?php echo (int) $nodCurrentMain; ?> drawings</span>
          </div>
          <div class="skai-card-body">
            <div class="skai-chart-shell">
        ...
        <div class="skai-card">
          <div class="skai-card-head skai-card-head--ember">
            ...
            <span class="skai-card-sub">Distance since last appearance for each main number (01&ndash;43)</span>
          </div>
          <div class="skai-card-body">
            <div class="skai-chart-shell">
              <div class="skai-chart-frame skai-chart-frame--tall">
      ...
      <div class="skai-note">
        The complete distribution is best used as a reference layer. The overview modules above are optimized for quick reading; this section is optimized for thorough review. A number with a high recency distance has not appeared in many recent drawings.
      </div>
    </div>
  </section>
  ...
  <section id="tables" class="skai-section" aria-labelledby="tables-title">
    <div class="skai-section-head">
      <div>
        ...
        <p class="skai-section-sub">
          Use the table for exact counts, recency, and personal tracking. Quick filters help narrow the view without losing full access to the complete dataset. Adjust the draw window below to broaden or tighten the analysis.
        </p>
      </div>
    </div>

    <div class="skai-controls">
      <form name="fqsearch" method="post" action="<?php echo $formActionVal; ?>#tables">
    ...
    <div class="skai-section-body">
      <div class="skai-two-col">
        ...
        <div class="skai-card">
          <div class="skai-card-head skai-card-head--horizon">
            Main numbers table
            <span class="skai-card-sub">Exact counts and recency for values 01&ndash;43 in <?php echo (int) $nodCurrentMain; ?> drawings</span>
          </div>
        ...
        <div class="skai-card">
          <div class="skai-card-head skai-card-head--radiant">
            Window snapshot
            <span class="skai-card-sub">Activity leaders across short and broad historical windows</span>
          </div>
          <div class="skai-card-body">
            <div class="skai-window-shift">
              <div class="skai-shift-panel">
                <p class="skai-shift-label">Analysis interpretation</p>
                <p class="skai-shift-text">This table shows how often each number in the 01&ndash;43 pool appeared within your selected window. Use filters to focus on the most active, quietest, or most recently drawn numbers. Track your selections using the checkboxes to keep a personal comparison set visible.</p>
              </div>

              <div class="skai-shift-panel">
                <p class="skai-shift-label">Top 5 in last 50 draws</p>
                <p class="skai-shift-text"><?php echo htmlspecialchars(leCommaList(array_slice($top50, 0, 5)), ENT_QUOTES, 'UTF-8'); ?></p>
              </div>

              <div class="skai-shift-panel">
                <p class="skai-shift-label">Top 5 in last 300 draws</p>
                <p class="skai-shift-text"><?php echo htmlspecialchars(leCommaList(array_slice($top300, 0, 5)), ENT_QUOTES, 'UTF-8'); ?></p>
              </div>
              ...
              <div class="skai-shift-panel">
                ...
                <p class="skai-shift-text"><?php echo htmlspecialchars(leCommaList(array_slice($windowShiftIn, 0, 3)), ENT_QUOTES, 'UTF-8'); ?></p>
              </div>
              <?php endif; ?>

              <?php if (!empty($windowShiftOut)) : ?>
              <div class="skai-shift-panel">
                <p class="skai-shift-label">More concentrated at 50 draws</p>
                <p class="skai-shift-text"><?php echo htmlspecialchars(leCommaList(array_slice($windowShiftOut, 0, 3)), ENT_QUOTES, 'UTF-8'); ?></p>
              </div>
              <?php endif; ?>
            </div>

            <div class="skai-note skai-note--spaced">
      ...
      <div class="skai-tool-grid">
        <article class="skai-tool">
          ...
          <div class="skai-tool-body">
            ...
            <a class="skai-tool-cta" href="<?php echo htmlspecialchars($skaiUrlVal, ENT_QUOTES, 'UTF-8'); ?>">Open SKAI Analysis</a>
          </div>
        </article>
        ...
        <article class="skai-tool">
          ...
          <div class="skai-tool-body">
            ...
            <a class="skai-tool-cta" href="<?php echo htmlspecialchars($aiUrlVal, ENT_QUOTES, 'UTF-8'); ?>">Open AI Predictions</a>
          </div>
        </article>
        ...
        <article class="skai-tool">
          ...
          <div class="skai-tool-body">
            ...
            <a class="skai-tool-cta" href="<?php echo htmlspecialchars($skipUrlVal, ENT_QUOTES, 'UTF-8'); ?>">Open Skip &amp; Hit</a>
          </div>
        </article>
      </div>

      <div class="skai-utility-grid">
        <a class="skai-utility-link" href="<?php echo htmlspecialchars($mcmcUrlVal, ENT_QUOTES, 'UTF-8'); ?>">MCMC Markov Analysis</a>
        <a class="skai-utility-link" href="<?php echo htmlspecialchars($heatmapUrlVal, ENT_QUOTES, 'UTF-8'); ?>">Heatmap Analysis</a>
        <a class="skai-utility-link" href="<?php echo htmlspecialchars($archiveUrlVal, ENT_QUOTES, 'UTF-8'); ?>">Lottery Archives</a>
        <a class="skai-utility-link" href="<?php echo htmlspecialchars($lowestUrlVal, ENT_QUOTES, 'UTF-8'); ?>">Lowest Number Analysis</a>
      </div>
    </div>
  </section>
  ...
  <section class="skai-section" aria-labelledby="method-title">
    ...
    <div class="skai-section-body">
      <div class="skai-method-note">
        <strong>Interpretation guidance:</strong> Frequency, recency, and spacing can provide useful context for reviewing draw history, but they should be treated as descriptive signals rather than guarantees. The purpose of this page is to make the recent behavior of
        <?php echo htmlspecialchars((string) ($stateName ?? '') . ' ' . (string) ($gName ?? ''), ENT_QUOTES, 'UTF-8'); ?>
        easier to understand, compare, and carry into deeper SKAI analysis. Every lottery draw is an independent random event; past patterns do not determine future outcomes.
      </div>
    </div>
  </section>
